...
class VisionPermutator(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=1000, dim=256, depth=12, heads=8):
        ...
        # Make transformer accept (B, seq, dim)
...
model = VisionPermutator(img_size=224, patch_size=16, num_classes=3, dim=256, depth=12, heads=8).to(device)
...
plt.title("Vision Permutator Confusion Matrix")
plt.tight_layout()

plt.savefig("vision_permutator_cm.png")
plt.show()
...
print("✅ VisionPermutator model saved successfully: vision_permutator.pth")
...
with open("metrics/vision_permutator_metrics.json", "w") as f:
    json.dump(vp_metrics, f, indent=2)

print("✅ Metrics saved to metrics/vision_permutator_metrics.json")
print("✅ Confusion matrix saved to metrics/vision_permutator_cm.png")
